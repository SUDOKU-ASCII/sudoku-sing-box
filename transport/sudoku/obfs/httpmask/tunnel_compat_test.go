...
func TestTunnelHTTPClientPreconnectsTLSHandshakes(t *testing.T) {
	...
	httpClient = newHTTPClient(target.Host, target.Host, target.Hostname(), "https", 4, false, nil, nil)
...
func TestTunnelHTTPClientPreconnectUsesCustomDialContext(t *testing.T) {
	...
	httpClient := newHTTPClient(target.Host, target.Host, target.Hostname(), "http", 4, false, nil, dialContext)
	t.Cleanup(httpClient.transport.close)
	httpClient.transport.transport.Proxy = nil
	...
	_, _ = io.Copy(io.Discard, resp.Body)
	_ = resp.Body.Close()
	if got := dialCount.Load(); got < tunnelPreconnectCount {
		t.Fatalf("custom dial count = %d, want at least %d", got, tunnelPreconnectCount)
	}
}

func TestTransportPoolConcurrentInitialization(t *testing.T) {
	pool := new(TransportPool)
	t.Cleanup(func() { _ = pool.Close() })

	const clients = 32
	transports := make(chan *tunnelHTTPTransport, clients)
	var group sync.WaitGroup
	group.Add(clients)
	for range clients {
		go func() {
			defer group.Done()
			client := newHTTPClient("example.com:80", "example.com:80", "example.com", "http", 4, true, pool, nil)
	...
	for transport := range transports {
		...
		if transport != first {
			t.Fatal("transport pool created more than one transport for the same key")
		}
	}
}

func TestPreparedConnPoolPrepareAddsPerSessionCapacity(t *testing.T) {
	...
	prepare := func() {
		...
		pool.prepare(context.Background(), tunnelPreconnectCount, dial, func() {
			close(done)
		})
		select {
		case <-done:
		case <-time.After(time.Second):
			t.Fatal("preconnect did not complete")
		}
	}

	prepare()
	prepare()
	if got := dialCount.Load(); got != 2*tunnelPreconnectCount {
		t.Fatalf("dial count = %d, want %d", got, 2*tunnelPreconnectCount)
	}
}

func TestPreparedConnPoolEnsureMaintainsTargetCapacity(t *testing.T) {
	pool := newPreparedConnPool()
	t.Cleanup(pool.close)
	...
	t.Cleanup(func() {
		...
		pool.ensure(context.Background(), tunnelPreconnectCount, dial)
		...
		for time.Now().Before(deadline) {
			...
			pool.mu.Unlock()
			if ready >= tunnelPreconnectCount && pending == 0 {
				return
			}
			time.Sleep(time.Millisecond)
		}
		t.Fatal("ensure did not complete")
	}
	...
	if got := dialCount.Load(); got != tunnelPreconnectCount {
		t.Fatalf("dial count = %d, want %d", got, tunnelPreconnectCount)
	}

	conn, ok, err := pool.take(context.Background())
	if err != nil || !ok {
		t.Fatalf("take prepared connection: ok=%v err=%v", ok, err)
	}
	_ = conn.Close()

	ensure()
	if got := dialCount.Load(); got != tunnelPreconnectCount+1 {
		t.Fatalf("dial count after refill = %d, want %d", got, tunnelPreconnectCount+1)
	}
}

func TestPreconnectDialerMaintainsSpareConnection(t *testing.T) {
	...
	dialer := newPreconnectDialer(listener.Addr().String(), listener.Addr().String(), "", nil, nil)
	defer dialer.close()
...
func TestPreparedConnPoolWaitReadyStopsOnTunnelClose(t *testing.T) {
	pool := newPreparedConnPool()
...
func TestPreparedConnPoolDoesNotReturnExpiredConnection(t *testing.T) {
	pool := newPreparedConnPool()
...
func TestRetryableHTTPTransportErrorTransportEOF(t *testing.T) {
...
func TestRetryDialDoesNotRetryAmbiguousTransportEOF(t *testing.T) {
	closed := make(chan struct{})
	var calls atomic.Int32
...
func TestStreamSplitPullRetriesTransportEOF(t *testing.T) {
	var calls atomic.Int32
...
func TestDialTunnelBidirectionalSmoke(t *testing.T) {
...
func TestDialTunnelHalfCloseDelayedResponse(t *testing.T) {
...
func TestDialTunnelDownlinkHalfCloseKeepsUplink(t *testing.T) {
	for _, mode := range []string{"stream", "poll", "auto"} {
		t.Run(mode, func(t *testing.T) {
			srv := NewTunnelServer(TunnelServerOptions{
				Mode:            "auto",
...
func TestDialTunnelWaitReadyConfirmsSplitSession(t *testing.T) {
	for _, mode := range []string{"stream", "poll", "auto"} {
		t.Run(mode, func(t *testing.T) {
			srv := NewTunnelServer(TunnelServerOptions{
				Mode:            "auto",
...
func TestDialTunnelSharedTransportSequentialSessions(t *testing.T) {
	srv := NewTunnelServer(TunnelServerOptions{
		Mode:            "auto",
		PathRoot:        compatPathRoot,
		AuthKey:         compatAuthKey,
	...
	for i := range 8 {
		...
		server := waitForTunnelConn(t, tunnelCh)
		assertLargeBidirectionalExchange(t, client, server, 2*1024*1024)
		_ = client.Close()
		_ = server.Close()
	}
	if dialCount.Load() == 0 {
...
func TestDialTunnelNewClientWithV047Server(t *testing.T) {
	tests := []struct {
		name     string
		dialMode string
		wireMode TunnelMode
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				auth := newTunnelAuth(compatAuthKey, 0)
				if !auth.verifyValue(authValue, test.wireMode, r.Method, path, time.Now()) {
				...
				case r.Method == http.MethodPost && path == "/api/v1/upload":
					...
					payload, err := decodeV047Payload(test.wireMode, body)
					if err != nil {
				...
				case r.Method == http.MethodGet && path == "/stream":
					...
					if !r.Close {
						reportHandlerError(handlerErr, w, "pull request did not disable carrier reuse")
						return
					}
					...
					_, _ = w.Write(encodeV047Payload(test.wireMode, buf))
...
func TestTunnelServerV047ClientBidirectional(t *testing.T) {
	for _, mode := range []TunnelMode{TunnelModeStream, TunnelModePoll} {
		t.Run(string(mode), func(t *testing.T) {
			...
			if !resp.Close {
				_ = resp.Body.Close()
				t.Fatal("pull response did not disable carrier reuse")
			}
			body, err = io.ReadAll(resp.Body)
			_ = resp.Body.Close()
			if err != nil {
...
func assertLargeBidirectionalExchange(t *testing.T, client, server net.Conn, size int) {
	t.Helper()
	clientPayload := bytes.Repeat([]byte{0x39, 0x71, 0xa5, 0xcc}, size/4)
	serverPayload := bytes.Repeat([]byte{0xe2, 0x18, 0x64, 0x8b}, size/4)

	errCh := make(chan error, 4)
	go func() {
		_ = client.SetWriteDeadline(time.Now().Add(10 * time.Second))
		errCh <- writeCompatPayload(client, clientPayload)
	}()
	go func() {
		_ = server.SetWriteDeadline(time.Now().Add(10 * time.Second))
		errCh <- writeCompatPayload(server, serverPayload)
	}()
	go func() {
		...
		if err == nil && !bytes.Equal(got, clientPayload) {
			err = fmt.Errorf("client payload mismatch")
		}
		errCh <- err
	}()
	go func() {
	...
	for range 4 {
		if err := <-errCh; err != nil {
			t.Fatalf("large bidirectional exchange: %v", err)
		}
	}
}

func writeCompatPayload(writer io.Writer, payload []byte) error {
	for len(payload) > 0 {
